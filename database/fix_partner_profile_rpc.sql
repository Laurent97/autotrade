-- Fix the get_partner_profile_by_user_id function
-- Drop existing function and recreate with correct return type

-- Step 1: Drop the existing function
DROP FUNCTION IF EXISTS get_partner_profile_by_user_id(UUID);
...
-- Step 3: Test the function with the correct user ID
SELECT * FROM get_partner_profile_by_user_id('e2731c06-58b4-4f37-96c7-f721af43263c');

-- Step 4: Also test with the other ID to see what happens
SELECT * FROM get_partner_profile_by_user_id('33235e84-d175-4d35-a260-1037ca5cfd0c');

-- Step 5: Check what the function should return for our partner
-- This should show the correct user data that will fix the ID mismatch
SELECT 
    ...
    pp.store_name,
    CASE 
        WHEN u.id = 'e2731c06-58b4-4f37-96c7-f721af43263c' THEN '✅ CORRECT ID'
        WHEN u.id = '33235e84-d175-4d35-a260-1037ca5cfd0c' THEN '❌ WRONG ID'
        ELSE '❓ OTHER'
    END as id_status
FROM users u
LEFT JOIN partner_profiles pp ON u.id = pp.user_id
